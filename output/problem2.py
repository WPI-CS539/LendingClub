...
def compute_a1(z1):
    ...
    try:
        ...
        for item in z1:
            try:
                a1.append(1 / (1 + math.exp(-item)))
            except:
                if item < 0:
                    a1.append(0)
                elif item > 0:
                    a1.append(1)
        a1 = np.asmatrix(np.asarray(a1).reshape(len(a1), 1))
    #########################################
    return a1 
...
#--------------------------
# train
def ann_train(X, Y,h=3, n_layers=3, alpha=0.01, n_epoch=100):
    '''
    ...
    for _ in range(n_epoch):
        ...
        for x,y in zip(X,Y):
            ...
            b2 = b2 - alpha * dL_db2
            #########################################
        print("Have Done " + str(_))
    return W1, b1, W2, b2

#--------------------------
def ann_predict(Xtest, W1,b1,W2,b2):
    '''
    ...
    for i, x in enumerate(Xtest):
        ...
        z2 = compute_z2(a1, W2, b2)
        P[i] = compute_a2(z2).T
        Y[i] = np.nonzero(P[i] == np.max(P[i]))[1][0]
